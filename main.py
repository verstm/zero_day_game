...
class map:
    def __init__(self, player):
        ...
        self.map.image = pygame.transform.scale(self.map.image,
                                                (self.map.image.get_width() * 3, self.map.image.get_height() * 3))
...
class Debug_Player:
    def __init__(self):
        ...
        self.right_armsprite2 = pygame.image.load(
            "Assets/Sprites/Character_parts/Body_parts/right_arm_rightfront.png").convert()
        ...
        self.left_legsprite2 = pygame.image.load(
            "Assets/Sprites/Character_parts/Body_parts/left_leg_rightfront.png").convert()
        ...
        self.right_legsprite2 = pygame.image.load(
            "Assets/Sprites/Character_parts/Body_parts/right_leg_rightfront.png").convert()
    ...
    def partsholding(self):
        ...
        if self.way == 3:
            ...
            self.left_leg.rect = self.left_leg.image.get_rect()
            self.left_leg.image = pygame.transform.scale(self.left_leg.image,
                                                         (self.left_leg.rect.width * self.t,
                                                          self.left_leg.rect.height * self.t))
            self.left_leg.rect = self.left_leg.image.get_rect()
...
class adminpanel:
    ...
    def update(self):
        ...
        if keystate[pygame.K_BACKQUOTE] and time.time() - self.lastacttime >= 1:
            ...
            if self.visible:
                self.player.movelock = 1
            else:
                self.player.movelock = 0
        if self.visible:
            keystate = pygame.key.get_pressed()
            if keystate[pygame.K_RETURN]:
                self.command = self.txt.value
                self.txt.value = ''
            ...
            self.txt.update(events)
            screen.blit(self.txt.surface, (100, 100))
